...
public class BoardTestSuite {
    ...
    @Test
    void testsAddTasksListAverageWorkingOnTask() {
        //Given
        Board project = prepareTestData();
        //When
        List<TaskList> inProgressTasks = new ArrayList<>();
        ...
        var sumTime = project.getTaskLists().stream()
                .filter(inProgressTasks::contains)
                .flatMap(tl -> tl.getTasks().stream())
                .map(task -> (Period.between(task.getCreated(), LocalDate.now())).getDays())
                .reduce(0, (sum, current) -> sum += current);
        var quantityTime = project.getTaskLists().stream()
                .filter(inProgressTasks::contains)
                .flatMap(tl -> tl.getTasks().stream())
                .map(task -> (Period.between(task.getCreated(), LocalDate.now())).getDays())
                .map(n -> 1)
                .reduce(0, (sum, current) -> sum += current);
        double average = sumTime/quantityTime;
        //Then
